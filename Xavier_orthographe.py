...
def t20():# Classement du top 20 des films    
    sql1 = 'SELECT primaryTitle AS Nom, averageRating AS Rating  FROM mieux_notes_movie ORDER BY averageRating DESC LIMIT 20;'
    #print("\nSQL1:", sql1)
...
def lfja():# Affiche le nom des films où a joué tel acteur 
    ...
    print("Le nom des film où a joué"+namefirstname1+" sont :")
    print(df5)
...
def gfr():# Affiche le genre des films que le réalisateur a réalisé et trace le camembert représentant les proportions de ses réalisations 
    ...
    print("Les genres de films réalisés par "+namefirstname2+" sont au nombre de :")
    print(df6)
    plt.pie(df6['COUNT(PrimaryTitle)'], startangle=90, autopct='%1.1f%%') 
    plt.suptitle("Proportion des genres de films réalisés par {}".format( namefirstname2))
...
def Nmfp(): #  Affiche la note moyenne obtenue pour les films d'origine :
    Country = input("De quel pays voudriez vous savoir la note moyenne obtenue ? ")
...
def nsup():# Affiche la liste des films avec une note supérieure à :
    ...
    print("\nSQL9:", sql9)
    print("La liste des films dont la note est supérieure à "+np+" sont :")
    df9 = pd.read_sql(sql9, cnx)
    print(df9)

def grfm():# Affiche les titres de films du genre : 
    gf = input("Rechercher les titres de films du genre : ")
    ...
    print("\nSQL10:", sql10)
    print("Les film du genre  "+gf+" sont : ")
...
def lf():# Affiche les langues du film :
    lfm = input("De quel film voulez-vous connaître les langues utilisées ? ")
    sql12='SELECT DISTINCT language FROM `title_akas` WHERE title="'+lfm+'" ORDER BY language DESC LIMIT 50;'    
    print("\nSQL12:", sql12)
    df12 = pd.read_sql(sql12, cnx)
    print(df12)
    
def npd(): # Affiche le nom des films que ... a réalisé  
    nfpd = input("Quel est le réalisateur dont vous voudriez voir les films produits ? ")
...
def t20tt(): #  Affiche Top 20 de tous les titres
    sql14 = 'SELECT primaryTitle AS Nom, averageRating AS Rating , titleType AS Type FROM mieux_notes_tout ORDER BY averageRating DESC LIMIT 20;'
    #print("\nSQL14:", sql14)
...
def t20ae(): # Affiche 20 meilleurs films d'une année spécifique de sortie
    print (f"Film / Notation / 20 meilleurs films d'une année spécifique de sortie")
    year = input("Année ? ")
...
def Ntn():
    ...
    6) Notation moyenne des films suivant un pays
    """)
...
def Actor():
    ...
    1) Quel sont les acteurs nés après telle année ?
